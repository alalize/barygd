...
if args.plot_final:
	assert args.resume_from != '', 'Option --plot-final requires specifying --resume-from.'
...
if args.resume_from == '':
	random_pd_matrix = lambda scale, dim: (lambda a, d: np.eye(d) + a.T.dot(a))(scale*(np.random.rand(dim, dim)-0.5), dim)
	...
	for gradient in potential_gradients:
		svgd_kernel.append_potential_gradient(gradient)
	model_solver = Solver(
