...
import subprocess
from utils import lawgd_dk, lawgd_gradient, dc, norm2
import datetime
...
if simul_params['film']: draw_freq = 1
if '2d' in simul_params['laws']: simul_params['dimension'] = 2

exp_name = 'bar_' + now_str + ''.join(['_{}:{}_'.format(k[:4], simul_params[k]) for k in ['dimension', 'g_conv', 'n_iterations', 'regularization', 'gd_step_size', 'laws']])
print('NAME:', exp_name)
...
if simul_params['laws'] == 'norm':
    mean_1, mean_2 = -2,4 
    std_1, std_2 = 1, 1
    potentials.append(lambda x: (x-mean_1)**2 / (2*std_1**2))
    potentials.append(lambda x: (x-mean_2)**2 / (2*std_2**2))
    grad_potentials.append(lambda x: (x - mean_1) / std_1**2)
    grad_potentials.append(lambda x: (x - mean_2) / std_2**2)
    lap_potentials.append(lambda x: 1/std_1**2)
    lap_potentials.append(lambda x: 1/std_2**2)
...
dk = [lawgd_dk(mesh, simul_params['dimension'], potentials[i], grad_potentials[i], \
...
metadata = dict(title='Bulle', artist='Le JeanThi Jacques')
writer = FFMpegWriter(fps=5, metadata=metadata)
mydpi=1
fig = plt.figure(figsize=(32,32))
l, = plt.plot([], [], 'k-o')
seq = [AL[i,:,:] for i in range(nb_images)]
with writer.saving(fig, "writer_test.mp4", nb_images):
    for i in range(nb_images):
        fig = plt.imshow(seq[i], cmap=cm.hot)
        writer.grab_frame()
"""
...
def plot_init(ion):
    ...
    if simul_params['film']:
        ...
        writer = FFMpegWriter(fps=15, metadata=film_desc)
        context['writer'] = writer

    return context   


def plot_intermediate(L, L_init, iteration, grads, context, simul_params):
    fig = context['fig']
    cols = context['cols']
    marks = context['marks']
    if simul_params['dimension'] == 1:
...
def lawgd_barygd(mesh, dk, simul_params):
    def bary_iteration(L):
        ...
        for alpha in range(simul_params['n_marginals']):
            for i in range(simul_params['n_samples']):
                gradk = lawgd_gradient(dk[alpha], L[alpha], simul_params['n_samples'], i)
                grad = \
                ...
                L_new[alpha, i] = L[alpha, i] - dcn
                grads[alpha, i] = grad - dcn
                L_new[alpha, i] = L_new[alpha, i] + grad
        diffgrad = L_new - L
    ...
    context = plot_init(ion=True)
    if simul_params['film']:
        context['writer'].setup(fig=context['fig'], outfile='../img/{}.mp4'.format(simul_params['film_name']), dpi=100)
...
init_samples, swarm, trajectories = lawgd_barygd(mesh, dk, simul_params)
if simul_params['dimension'] == 1:
...
if simul_params['laws'] in ['norm', 'norm-arctan', 'norm-exp']:
    ...
    for k in range(simul_params['n_samples']):
        ...
        axs[1].plot(path[:, 0], path[:, 1], '-', c='black', markeredgecolor='gold', alpha=0.2, zorder=1)
    ...
    fig.savefig('../img/lawgd_{}.png'.format(exp_name))
    plt.show()
