...
import subprocess
from utils_2 import lawgd_dk, lawgd_gradient, dc, norm2
import datetime
...
def lawgd_barygd(mesh, dk, simul_params):
    def bary_iteration(L, iteration=None):
        L_new = np.copy(L)
    ...
    for iteration in range(simul_params['n_iterations']):
        ...
        L, grads, diffgrad = bary_iteration(L, iteration=iteration if simul_params['time_to_c'] < np.inf and iteration < simul_params['time_to_c'] else None)
        particle_trajectories.append(np.copy(L))
