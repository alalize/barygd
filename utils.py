...
def sample_uniform_dsphere(dim, num_samples):
    x = np.random.normal(size=(num_samples, dim))
    u = np.array([x[i]/norm(x[i]) for i in range(num_samples)])

    return u


def norm2(x):
    if x.size == 1: return x**2
    return x.dot(x)
...
def dc(x, lambdas, convex=False):
    ...
    for k in range(n):
        dx[k] = 2*lambdas[k]*(x[k] - np.sum([lambdas[i]*x[i] for i in range(n)], axis=0))
...
def dcoulomb(x, lambdas, convex=False):
    ...
    for k in range(x.shape[0]):
        dx[k] = np.sum([1/norm2(x[k] - x[j]) if j < k else (0 if j == k else -1/norm2(x[k] - x[j])) for j in range(x.shape[0])])
    dx = (dx + 1e-10) / x.shape[0] # equal weights
    return dx
...
def barycenter_of_gaussians(means, covariances, weights, threshold=1e-9, maxiter=1000):
    dim = 1 if np.isscalar(means[0]) else means[0].size
    baryenter_mean = sum([weight_i*mean_i for (weight_i, mean_i) in zip(weights, means)])

    def fixed_point_func(s):
        v = sum([wi*sqrtm(sqrtm(s).dot(si).dot(sqrtm(s))) for (wi, si) in zip(weights, covariances)])
        return v

    thresholded_out = False
    bary_covariance = np.eye(dim)
    for i in range(maxiter):
        old, bary_covariance = bary_covariance, fixed_point_func(bary_covariance)

        if np.max(np.abs(bary_covariance - old)) < threshold:
            thresholded_out = True
            print('(barycenter_of_gaussians)  Thresholded out after {} iterations'.format(i))
            break
    
    return baryenter_mean, bary_covariance, thresholded_out
...
def laws_2norm_family(means, stds, bary_weight1, simul_params):
    ...
    if simul_params['laws'] == 'norm-arctan':
        ...
        pushforward = lambda t: np.exp(-(np.log(t)-means[0])**2/(2*stds[0]**2)) / (t*np.sqrt(2*np.pi)*stds[0])
    else:
...
if __name__ == '__main__':
    ...
    test_c = False
    test_mmd = False
    test_dsphere_sample = True

    if test_dsphere_sample:
        x = sample_uniform_dsphere(3, 10)
        print('Uniform d-sphere samples:\n', x)
